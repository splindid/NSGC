...
from .my_DataStatistic import DataStatistic
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.pp = self.cfg.MODEL.ROI_RELATION_HEAD.CAUSAL.REPROCESS
        if self.pp:
            self.ds = DataStatistic(self.cfg)
            # 关键：将 ds 内部的 corrector 注册为子模块，否则参数无法训练
            self.add_module("ds_corrector", self.ds.corrector)
        # parameters
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.training:
            ...
            # In inference mode, if USE_GT_BOX=True, also use GT boxes (for SGCls/PredCls evaluation)
            if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                with torch.no_grad():
                    proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.gtbox_relsample(proposals, targets)
            else:
        ...
        if self.pp:
             relation_logits = self.ds.correction(refine_logits, relation_logits, rel_pair_idxs, proposals, roi_features, union_features)
        # for test
