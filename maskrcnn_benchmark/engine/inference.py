...
def inference(
    ...
    if load_prediction_from_cache:
        # 显式设置 weights_only=False 以加载包含 BoxList 的结果
        predictions = torch.load(os.path.join(output_folder, "eval_results.pytorch"), map_location=torch.device("cpu"), weights_only=False)['predictions']
        
        if cfg.MODEL.ROI_RELATION_HEAD.CAUSAL.REPROCESS:
            logger.info("Applying post-processing (DataStatistic.correction) to cached predictions...")
            from maskrcnn_benchmark.modeling.roi_heads.relation_head.my_DataStatistic import DataStatistic
            ds = DataStatistic(cfg)
            
            # 获取所有 predictions 的关系数据并进行批量修正
            # 这里我们需要构造 DataStatistic.correction 所需的参数格式
            rel_pair_idxs = [boxlist.get_field("rel_pair_idxs") for boxlist in predictions]
            relation_logits = torch.cat([boxlist.get_field("pred_rel_scores") for boxlist in predictions], dim=0)
            
            # 注意：某些模型可能没存 refine_logits，这里做个保护
            if predictions[0].has_field("pred_scores"):
                refine_logits_list = [boxlist.get_field("pred_scores") for boxlist in predictions]
                # 检查维度，如果是 [N] 则需要变成 [N, 1]
                if refine_logits_list[0].dim() == 1:
                    refine_logits = torch.cat([x.unsqueeze(1) for x in refine_logits_list], dim=0)
                else:
            ...
            # 执行修正
            corrected_logits = ds.correction(refine_logits, relation_logits, rel_pair_idxs, predictions)
            
            # 将修正后的结果写回 predictions
            offset = 0
            for boxlist in predictions:
                num_rel = len(boxlist.get_field("rel_pair_idxs"))
                boxlist.add_field("pred_rel_scores", corrected_logits[offset : offset + num_rel])
                offset += num_rel
